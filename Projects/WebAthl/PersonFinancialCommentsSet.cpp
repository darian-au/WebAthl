// PersonFinancialCommentsSet.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CPersonFinancialCommentsSet

IMPLEMENT_DYNAMIC(CPersonFinancialCommentsSet, CRecordsetEx)

CPersonFinancialCommentsSet::CPersonFinancialCommentsSet(CDatabase* pdb)
	: CRecordsetEx(pdb)
{
	//{{AFX_FIELD_INIT(CPersonFinancialCommentsSet)
	m_PersonID = 0;
	m_LastName = _T("");
	m_FirstName = _T("");
	m_MiddleName = _T("");
	...
	m_Comments_PersonID = 0;
	m_Comments_CommentNo = 0;
	m_Comments_Type = _T("");
	m_Comments_Comment = _T("");
...
CString CPersonFinancialCommentsSet::GetDefaultConnect()
{
	return _T("");
}

CString CPersonFinancialCommentsSet::GetDefaultSQL()
{
	return _T("[Person],[Financial],[RegistrationType],[Organisation],[PersonComments]");
}

void CPersonFinancialCommentsSet::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CPersonFinancialCommentsSet)
	...
	RFX_Long(pFX, _T("[PersonComments].[PersonID]"), m_Comments_PersonID);
	RFX_Long(pFX, _T("[PersonComments].[CommentNo]"), m_Comments_CommentNo);
	RFX_Text(pFX, _T("[PersonComments].[Type]"), m_Comments_Type);
	RFX_Text(pFX, _T("[PersonComments].[Comment]"), m_Comments_Comment);
...
/////////////////////////////////////////////////////////////////////////////
// CPersonFinancialCommentsSet diagnostics

#ifdef _DEBUG
void CPersonFinancialCommentsSet::AssertValid() const
{
	CRecordsetEx::AssertValid();
}

void CPersonFinancialCommentsSet::Dump(CDumpContext& dc) const
{
