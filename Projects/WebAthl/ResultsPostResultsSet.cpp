// ResultsPostResultsSet.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CResultsPostResultsSet

IMPLEMENT_DYNAMIC(CResultsPostResultsSet, CRecordsetEx)

CResultsPostResultsSet::CResultsPostResultsSet(CDatabase* pdb)
	: CRecordsetEx(pdb)
{
	//{{AFX_FIELD_INIT(CResultsPostResultsSet)
	m_SeriesID = 0;
	m_EventID = 0;
	m_PersonID = 0;
	m_DivisionActual = 0;
	...
	m_Post_EventID = 0;
	m_Post_PersonID = 0;
	m_Post_GroupIDActual = 0;
	m_Post_DivisionActual = 0;
	m_Post_GrossPerformanceHOUR = 0;
	m_Post_GrossPerformanceMIN = 0;
	m_Post_GrossPerformanceSEC = 0.0f;
	m_Post_Place = 0;
	m_Post_AgeGroup = 0;
	m_Post_ChestNumber = _T("");
	m_Post_NewGroupID = 0;
	m_Post_NewGroupMessage = _T("");
	m_Post_Points = 0;
...
CString CResultsPostResultsSet::GetDefaultConnect()
{
	return _T("");
}

CString CResultsPostResultsSet::GetDefaultSQL()
{
	return _T("[Results]");
}

void CResultsPostResultsSet::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CResultsPostResultsSet)
	...
	RFX_Long(pFX, _T("[PostResults].[SeriesID]"), m_Post_SeriesID);
	RFX_Long(pFX, _T("[PostResults].[EventID]"), m_Post_EventID);
	RFX_Long(pFX, _T("[PostResults].[PersonID]"), m_Post_PersonID);
	RFX_Long(pFX, _T("[PostResults].[GroupIDActual]"), m_Post_GroupIDActual);
	RFX_Long(pFX, _T("[PostResults].[DivisionActual]"), m_Post_DivisionActual);
	RFX_Long(pFX, _T("[PostResults].[GrossPerformanceHOUR]"), m_Post_GrossPerformanceHOUR);
	RFX_Long(pFX, _T("[PostResults].[GrossPerformanceMIN]"), m_Post_GrossPerformanceMIN);
	RFX_Double(pFX, _T("[PostResults].[GrossPerformanceSEC]"), m_Post_GrossPerformanceSEC);
	RFX_Long(pFX, _T("[PostResults].[Place]"), m_Post_Place);
	RFX_Long(pFX, _T("[PostResults].[AgeGroup]"), m_Post_AgeGroup);
	RFX_Text(pFX, _T("[PostResults].[ChestNumber]"), m_Post_ChestNumber);
	RFX_Long(pFX, _T("[PostResults].[NewGroupID]"), m_Post_NewGroupID);
	RFX_Text(pFX, _T("[PostResults].[NewGroupMessage]"), m_Post_NewGroupMessage);
	RFX_Long(pFX, _T("[PostResults].[Points]"), m_Post_Points);
	//}}AFX_FIELD_MAP
}

/////////////////////////////////////////////////////////////////////////////
// CResultsPostResultsSet diagnostics

#ifdef _DEBUG
void CResultsPostResultsSet::AssertValid() const
{
	CRecordsetEx::AssertValid();
}

void CResultsPostResultsSet::Dump(CDumpContext& dc) const
{
